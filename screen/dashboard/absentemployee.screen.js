...
export class AbsentEmployeeList extends Component {
    constructor(props) {
        super(props) 
        this.state = {
            auth: null,
    ...
    getEmployeeListData(auth, id, url, year){
        // APIs.getAbsentEmployeeListData(url, auth, id, year)
        // .then((res) => {
          ...
          <SafeAreaView style = {{flex: 1}}>
              ...
              <View style={{ height: 60, width: '100%', backgroundColor: color.light, alignItems: 'center', flexDirection: 'row' }}>
                        ...
                        <Text style={{
                            ...
                            fontFamily: 'Nunito'
                        }}>Today Absent Employees</Text>
                    </View>
                    <Content style = {{flex: 1}}>
                    {
                            empLists.map((emp, index) => {
                                return (
                                    <View key = {index} style={{ marginLeft: 15, marginRight: 15, marginTop: 15, borderRadius: 8, backgroundColor: color.light, padding: 10, alignItems: 'center', flexDirection: 'row', borderWidth: 0.3, borderColor: color.placeHolder }}>
                                        ...
                                        <View style={{ marginLeft: 10 }}>
                                            <Text style={{ fontSize: 14, fontFamily: 'Nunito-Bold' }}>Employee Name</Text>
                                            <Text style={{ marginTop: 5, fontSize: 13, fontFamily: 'Nunito', color: '#656565' }}>Position, Department Name</Text>
                                            <Text style={{ marginTop: 8, fontSize: 14, fontFamily: 'Nunito', color: '#A5A5A5' }}>Location</Text>
                                        </View>
                                    </View>
                                )
                            })
